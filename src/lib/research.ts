...
import rawAbstracts from "@/data/pubmed-abstracts.json";

/** PubMed abstract record — structured output of tools/fetch_pubmed_abstracts.py.
 *  Abstracts are MEDLINE data from the U.S. National Library of Medicine and
 *  render with visible source attribution on the /research/[slug] page. */
...
interface AbstractsFile {
  _meta?: Record<string, unknown>;
  abstracts: Record<string, PubmedAbstract>;
}

const ABSTRACTS_BY_PMID: Record<string, PubmedAbstract> =
  (rawAbstracts as AbstractsFile).abstracts ?? {};
...
export type StudyWithSlug = Study & {
  ...
  supplementId?: string;
  /** PubMed ID — extracted from webapp `pmid` field or parsed out of the
   *  guide-evidence `url`. Used to look up the abstract + surface the PMID
   *  chip on the detail page. */
  pmid?: string;
  /** DOI when present in the webapp study record. */
  doi?: string;
  /** PubMed abstract for the study, when available. Populated at module load
   *  via pubmed-abstracts.json (generated by tools/fetch_pubmed_abstracts.py).
   *  Not every PMID has an abstract body (letters/editorials don't), so the
...
const guideEntries: StudyWithSlug[] = Object.entries(studies).map(
  ([id, study]) => {
    const pmid = pmidOf(study.url) ?? undefined;
    return {
      ...study,
      id,
      slug: toSlug(id),
      pmid,
      abstract: pmid ? ABSTRACTS_BY_PMID[pmid] : undefined,
    };
  },
);
  ...
  for (const s of value as WebappStudy[]) {
    ...
    const url = s.pmid
      ...
      : s.doi
        ? `https://doi.org/${s.doi}`
        : "";
    const pmid = s.pmid ? String(s.pmid) : undefined;
    webappEntries.push({
      id,
      slug: toSlug(id),
