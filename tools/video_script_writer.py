...
# Narration budget, MEASURED rather than guessed. This was 2.6 words/second,
# which was an estimate — rendering seven beats through a real voice and timing
# the output gave 109 words in 54.0s of speech, or 2.0. The gap is not small:
# at 2.6 the hook was trimmed to fit three seconds and took 8.6 to read, so
# every downstream timing was wrong by roughly a third.
#
# Clinical text runs slower than ordinary prose — polysyllabic drug names and
# unit strings ("600 mg/day standardized extract") cost more time per word than
# the count suggests, which is why a general-purpose estimate was optimistic.
WORDS_PER_SEC = 2.0
ON_SCREEN_WORDS = 8
...
def beat(label: str, on_screen: str, voice: str, field: str, cap_seconds: float) -> dict:
    """One beat, timed by ITS OWN CONTENT rather than a slot decided up front.

    The first version carried fixed ranges ("0:00–0:03") and trimmed toward
    them. Rendering it through a real voice showed the hook taking 8.6 seconds
    in its three-second slot — and the trim could not help, because a single
    sentence is never cut (cutting a clinical sentence in half is worse than
    letting it run). Fixed slots were therefore a fiction the script asserted
    and the video contradicted.

    So `cap_seconds` now bounds MULTI-sentence fields only, and every beat
    reports the duration its own words imply. A single over-long sentence is
    kept whole and its real cost is stated rather than hidden.
    """
    fitted, dropped = fit(voice.strip(), cap_seconds)
    est = round(len(fitted.split()) / WORDS_PER_SEC, 1)
    return {
        "label": label,
...
def script_for(row: dict) -> dict:
    ...
    beats = [
        beat("HOOK", f"{a} + {b}", _txt(row, "summary"), "summary", 6),
        beat("WHAT HAPPENS", sev.upper() if sev else "INTERACTION",
             _txt(row, "details"), "details", 10),
        # No trailing period appended — the source field already ends in one, and
        # the first run rendered "…eliminate the risk.." on screen.
        beat("WHY", mechanism, f"The mechanism: {mechanism}", "mechanism", 10),
        beat("WHAT TO DO", "What to do", _txt(row, "recommendation"), "recommendation", 10),
    ]
    if _txt(row, "timing_advice"):
        beats.append(beat("TIMING", "Timing", _txt(row, "timing_advice"), "timing_advice", 7))
    if unknown:
        ...
        beats.append(beat("WHAT WE DON'T KNOW", "What we don't know",
                          unknown, "what_we_dont_know", 8))
    beats.append(
        beat("CTA", "Check your own stack — free",
             "Check your whole stack for interactions like this one. Free, no account needed.",
             "(fixed)", 6)
    )

    # Timings run off the content that survived, so the script and a real read
    # agree instead of the script asserting a slot the voice cannot hit.
    clock = 0.0
    for bt in beats:
        bt["t"] = f"{mmss(clock)}–{mmss(clock + bt['est_seconds'])}"
        clock += bt["est_seconds"]
...
def render(script: dict, score: float) -> str:
    out = [
        "─" * 74,
        f"{script['pair']}   [{script['severity'] or 'n/a'} · evidence: {script['evidence'] or 'unstated'}"
        f" · score {score} · ~{script['runtime_seconds']}s]",
        "─" * 74,
    ]
