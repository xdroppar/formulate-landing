...
- **`.env.local` points `NEXT_PUBLIC_API_URL` at PRODUCTION** (`formulate-api.onrender.com`). `npm run dev` therefore fires real analytics events at the prod `/events` stream. Before any local click-through, load test, or browser automation, override it in **`.env.development.local`** (higher Next precedence than `.env.local`) and confirm by watching the browser's actual network requests hit `localhost` — not by assuming a shell env var won. Otherwise the synthetic traffic lands in the very funnel you are trying to measure. Note `npm run dev` also falls through to **port 3001** when 3000 is taken, so check which app you actually loaded.
- **Instrumentation is verified from data, not by hand.** `GET /api/v1/events/integrity` (admin, in formulate-api) flags paths where `page_leave` outnumbers `page_view` — impossible for real users and the signature of a duplicate listener. This is how the 2026-08-11 `PageTracker` double-fire was generalised: flush was bound to BOTH `pagehide` and `visibilitychange`, which browsers fire on the same unload. If you add another departure hook, keep the one-leave-per-view guard (`sentLeaveRef`) intact.
